...
def decode_vector_to_ann_params(x):
    ...
    alpha = 10 ** (-6 + x[2] * 5)
    lr = 10 ** (-5 + x[3] * 3.5)
    activation = "relu" if x[4] < 1/3 else ("tanh" if x[4] < 2/3 else "logistic")
    num_layers = 1 if x[5] < 1/3 else (2 if x[5] < 2/3 else 3)
    dropout = float(0.5 * x[6])
    batch_size = 16 if x[7] < 0.25 else (32 if x[7] < 0.5 else (64 if x[7] < 0.75 else 128))
...
def format_extreme_value(val):
    """Format extreme values for display in tables"""
    if pd.isna(val) or not np.isfinite(val):
        return "N/A"
    abs_val = abs(val)
    if abs_val > 1e15 or (abs_val < 1e-15 and abs_val > 0):
        # For extremely large/small values, use shorter notation
        return f"{val:.2e}"
    elif abs_val > 1e6:
        return f"{val:.4e}"
    else:
...
def is_extreme_value(val, threshold=1e15):
    """Check if a value is extreme (overflow/underflow)"""
    if pd.isna(val) or not np.isfinite(val):
        return True
    return abs(val) > threshold


def filter_extreme_data(df, metric, threshold=1e15):
    """Filter out rows with extreme values in the specified metric"""
    mask = df[metric].apply(lambda x: not is_extreme_value(x, threshold))
    return df[mask].copy()
...
def cohens_d(group1, group2):
    ...
    if n1 == 0 or n2 == 0:
        return 0
    # Filter out extreme values before calculation
    g1 = [x for x in group1 if np.isfinite(x) and abs(x) < 1e15]
    g2 = [x for x in group2 if np.isfinite(x) and abs(x) < 1e15]
    if len(g1) == 0 or len(g2) == 0:
        return 0
    var1, var2 = np.var(g1, ddof=1), np.var(g2, ddof=1)
    pooled_std = np.sqrt(((len(g1)-1)*var1 + (len(g2)-1)*var2) / (len(g1)+len(g2)-2))
...
def calculate_rankings(df, metric='best_value'):
    ...
    for group in df[group_col].unique():
        ...
        for seed in group_data['seed'].unique():
            seed_data = group_data[group_data['seed'] == seed].sort_values(metric)
            seed_data['rank'] = range(1, len(seed_data) + 1)
            rankings.append(seed_data[['optimizer', 'rank']])

    all_rankings = pd.concat(rankings)
    avg_rankings = all_rankings.groupby('optimizer')['rank'].mean().sort_values()
...
def display_ann_convergence_page(df):
    ...
    if selected_optimizer != 'All Optimizers':
        filtered_df = filtered_df[filtered_df['optimizer'] == selected_optimizer]
    ...
    if view_mode in ["Convergence History", "Both"]:
        ...
        for _, row in filtered_df.iterrows():
            ...
            if len(conv_hist) > 0:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=label,
                    line=dict(
                        color=PUBLICATION_COLORS['primary'][color_idx % len(PUBLICATION_COLORS['primary'])],
        ...
        if has_data:
            ...
            st.caption("ℹ️ **Note:** Absolute values are used for plotting. Extreme outliers (penalty values) are automatically filtered for better visualization.")
        else:
            st.warning("⚠️ No convergence history data available for the selected filters. The convergence history may be empty or contain invalid values.")
            ...
            with st.expander(title_text, expanded=(len(filtered_df) == 1)):
                ...
                with col2:
                    ...
                    if len(best_pos) == 10:  # Must be 10-dimensional
                        ...
                        st.info(f"Best position has {len(best_pos)} dimensions (expected 10). Cannot decode to hyperparameters.")
                        # Show raw values as fallback
                        if len(best_pos) > 0:
                            param_names = [f"Parameter {i+1}" for i in range(len(best_pos))]
                            params_data = {
                ...
                if pd.notna(row.get('opt_params', None)) and row['opt_params']:
                    ...
                    if opt_params:
                        ...
                        st.dataframe(styled_opt, use_container_width=True, hide_index=True)
...
def display_ann_statistical_analysis(df):
    ...
    if analysis_type == "Performance Summary":
        display_ann_performance_summary(filtered_df, metric, metric_labels[metric], scope_title)
    elif analysis_type == "Wilcoxon Test":
        display_ann_wilcoxon_test(filtered_df, metric, metric_labels[metric], baseline, scope_title)
    elif analysis_type == "Effect Size Analysis":
        display_ann_effect_size(filtered_df, metric, metric_labels[metric], baseline, scope_title)
    elif analysis_type == "Win/Tie/Loss Analysis":
        display_ann_win_tie_loss(filtered_df, metric, metric_labels[metric], baseline, scope_title)
    elif analysis_type == "Ranking Analysis":
        display_ann_ranking(filtered_df, metric, metric_labels[metric], scope_title)
...
def display_ann_performance_summary(df, metric, metric_label, scope_title):
    ...
    for idx, optimizer in enumerate(sorted(df['optimizer'].unique())):
        ...
        fig.add_trace(go.Box(
            ...
            name=optimizer,
            marker=dict(
                color=PUBLICATION_COLORS['primary'][idx % len(PUBLICATION_COLORS['primary'])],
    ...
    fig = apply_publication_style(
        ...
        yaxis_type='log' if metric in ['mse', 'efficiency_mse_time', 'efficiency_mse_evals'] else 'linear',
        trace_type='box'
    )
...
def display_ann_wilcoxon_test(df, metric, metric_label, baseline, scope_title):
    ...
    st.markdown(f"### Wilcoxon Signed-Rank Test: {baseline} vs Others ({scope_title})")
    ...
    **Significance Level:** α = 0.05
    ...
    for competitor in competitors:
        ...
        if len(baseline_vals) > 0:
            statistic, p_value = perform_wilcoxon_test(baseline_vals, competitor_vals)
    ...
    if sig_count > 0:
        st.success(f"✅ **{baseline}** shows statistically significant improvement over **{sig_count}/{total_count}** algorithms (p < 0.05)")
    else:
        st.warning(f"⚠️ No statistically significant differences detected at α = 0.05")


def display_ann_effect_size(df, metric, metric_label, baseline, scope_title):
    """Display effect size analysis for ANN"""
    st.markdown(f"### Effect Size Analysis: {baseline} vs Others ({scope_title})")
    ...
    **Positive d**: Competitor performs better
    ...
    effect_df = pd.DataFrame(effect_sizes).sort_values('Magnitude', ascending=False)
    ...
    large_effects = len(effect_df[(effect_df['Interpretation'] == 'Large') & (effect_df["Cohen's d"] < 0)])
    
    if large_effects > 0:
        st.success(f"✅ **{baseline}** shows **large practical significance** over **{large_effects}** algorithm(s)")


def display_ann_win_tie_loss(df, metric, metric_label, baseline, scope_title):
    """Display win/tie/loss analysis for ANN"""
    st.markdown(f"### Win/Tie/Loss Analysis: {baseline} vs Others ({scope_title})")
    ...
    fig.add_trace(go.Bar(name='Wins', x=competitors, y=wins,
                         marker=dict(color='#00AA00', line=dict(color='black', width=2)),
                         text=wins, textposition='auto'))
    fig.add_trace(go.Bar(name='Ties', x=competitors, y=ties,
                         marker=dict(color='#CCCC00', line=dict(color='black', width=2)),
                         text=ties, textposition='auto'))
    fig.add_trace(go.Bar(name='Losses', x=competitors, y=losses,
                         marker=dict(color='#CC0000', line=dict(color='black', width=2)),
...
def display_ann_ranking(df, metric, metric_label, scope_title):
    ...
    **Lower rank = Better performance**
    ...
    st.success(f"🏆 **{ranking_df.iloc[0]['Optimizer']}** achieves the best average ranking!")
...
if app_section == "🧠 ANN":
    ...
    if ann_file is None:
        ...
        st.info("Please ensure there is a folder starting with 'ANN_' containing 'benchmark_master.csv'")
    else:
        try:
...
# ========================================
    ...
    if page == "Function Viewer":
        ...
        if view_mode == "Convergence History":
            ...
            # Info note about data processing
            st.caption("ℹ️ **Note:** Absolute values are used for plotting to ensure log scale compatibility.")
    ...
    # ========================================
        ...
        baseline = st.sidebar.selectbox("Baseline Algorithm",
                                        sorted(benchmark_df["optimizer"].unique()),
        ...
        if analysis_type == "Overall Ranking":
            ...
            # Option to filter extreme values
            filter_extremes = st.checkbox(
                "Filter extreme values (|value| > 1e15)", 
                value=True,
                help="Remove data points with extreme overflow/underflow values"
            )
            
            if filter_extremes:
                analysis_df = filter_extreme_data(filtered_df, metric, threshold=1e15)
                excluded_count = len(filtered_df) - len(analysis_df)
                if excluded_count > 0:
                    st.info(f"ℹ️ Filtered {excluded_count} data points with extreme values")
            else:
                analysis_df = filtered_df

            rankings = calculate_rankings(analysis_df, metric)
            ...
            fig.add_trace(go.Bar(
                ...
                y=rankings.values,
                marker=dict(
                    color=[colors[i % len(colors)] for i in range(len(rankings))],
                    line=dict(color='black', width=1)
                ),
        ...
        # Win/Tie/Loss Analysis
            ...
            st.subheader(f"⚔️ Win/Tie/Loss: {baseline} vs Others ({scope_title})")
            ...
            wtl_results = win_tie_loss_analysis(analysis_df, baseline, metric)
            ...
            with col1:
                st.metric("Total Wins", f"{total_wins}", help=f"{baseline} wins")
            with col2:
        ...
        # Pairwise Comparison
            ...
            **Wilcoxon Signed-Rank Test**: Tests if there is a significant difference between paired samples.
            ...
            filter_extremes = st.checkbox(
                "Filter extreme values (|value| > 1e15)", 
                value=True,
                help="Remove data points with extreme overflow/underflow values for more meaningful statistical analysis"
            )
            ...
            if filter_extremes:
                ...
                excluded_count = len(filtered_df) - len(analysis_df)
                if excluded_count > 0:
                    st.info(f"ℹ️ Filtered {excluded_count} data points with extreme values (|value| > 1e15)")
            else:
                analysis_df = filtered_df

            competitors = [
                opt for opt in analysis_df["optimizer"].unique() if opt != baseline]
            ...
            for competitor in competitors:
                ...
                for func in analysis_df['function'].unique():
                    func_data = analysis_df[analysis_df['function'] == func]
                    ...
                    for seed in func_data['seed'].unique():
                        ...
                        if len(base_val) > 0 and len(comp_val) > 0:
                            ...
                            if np.isfinite(base_val[0]) and np.isfinite(comp_val[0]):
                                baseline_vals.append(base_val[0])
                                competitor_vals.append(comp_val[0])
                ...
                if len(baseline_vals) > 0:
                    ...
                    results.append({
                        'Competitor': competitor,
                        'n': len(baseline_vals),
                        f'{baseline} Mean': format_extreme_value(mean_baseline),
                        f'{competitor} Mean': format_extreme_value(mean_competitor),
